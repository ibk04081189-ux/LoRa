...
def path_loss_db(d):
    ...
    # 1. 基準距離（D_BP）までのパスロス（2.0勾配）
    loss_near = L0_DB + 20.0 * np.log10(d / D0)
    
    # 2. 基準距離（D_BP）を超えたあとのパスロス（3.0勾配）
    l_at_bp = L0_DB + 20.0 * np.log10(D_BP / D0)
    loss_far = l_at_bp + 30.0 * np.log10(d / D_BP)
    
    return np.where(d <= D_BP, loss_near, loss_far)
